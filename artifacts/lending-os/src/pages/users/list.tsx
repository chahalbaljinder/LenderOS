...
import { useListUsers, useCreateUser, useUpdateUser, useGetUser } from "@workspace/api-client-react";
import { Link, useLocation } from "wouter";
import { ArrowLeft, Loader2, Shield, UserPlus, UserCheck, Edit, Trash2, Mail, Shield as ShieldIcon, Key, UserCog, Eye, MoreHorizontal, Plus, Search, Filter, AlertTriangle } from "lucide-react";
...
export default function UserManagementPage() {
  ...
  const [formData, setFormData] = useState({ email: "", firstName: "", lastName: "", phone: "", role: "tenant_admin", isActive: true });
  ...
  const createMutation = useCreateUser({
    onSuccess: () => {
      setShowModal(false);
      setFormData({ email: "", firstName: "", lastName: "", phone: "", role: "tenant_admin", isActive: true });
      refetch();
    },
  ...
  const roleColors: Record<string, string> = {
    ...
    auditor: "bg-gray-500/20 text-gray-400 border-gray-500/30",
    compliance_officer: "bg-gray-500/20 text-gray-400 border-gray-500/30",
  };
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="font-mono text-sm text-zinc-400">Manage platform users and roles</p>
            </div>
          </div>
          <button
            onClick={handleNew}
