...
import { FileText, CheckCircle, RefreshCcw, Wallet } from "lucide-react";
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';

export default function TenantDashboard() {
  const { data: user } = useGetMe();
  const tenantId = user?.tenantId;
  const { data: dashboard, isLoading } = useGetTenantDashboard({ period: '30d' }, { query: { enabled: !!tenantId } });
        ...
        <div className="flex justify-between items-end mb-8">
          ...
          <div className="hidden md:flex gap-2">
            {['7D', '30D', '90D', 'YTD'].map(p => (
              <button key={p} className={`px-3 py-1 font-mono text-xs border ${p === '30D' ? 'border-primary text-primary bg-primary/10' : 'border-border text-zinc-500 hover:text-white'}`}>
                {p}
              </button>
            ))}
          </div>
          ...
          <div className="lg:col-span-2 bg-card border border-border p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-4">
                <div className="flex items-center gap-2"><div className="w-2 h-2 bg-primary"></div><span className="text-xs font-mono">Disbursed</span></div>
                <div className="flex items-center gap-2"><div className="w-2 h-2 bg-zinc-600"></div><span className="text-xs font-mono">Collected</span></div>
              </div>
            </div>
            <div className="h-72 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={mockTrendData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <XAxis dataKey="date" stroke="hsl(240 10% 40%)" fontSize={12} tickLine={false} axisLine={false} />
                  ...
                  />
                  <Bar dataKey="disbursals" fill="hsl(160 100% 40%)" radius={[2, 2, 0, 0]} />
                  <Bar dataKey="collections" fill="hsl(240 10% 30%)" radius={[2, 2, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
